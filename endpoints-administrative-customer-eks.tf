...
resource "aws_apigatewayv2_integration" "eks_integration_administrative_customer" {
...
resource "aws_apigatewayv2_route" "get_customers" {
  ...
  route_key          = "GET /v1/administrative/customer"
  target             = "integrations/${aws_apigatewayv2_integration.eks_integration_administrative_customer.id}"
